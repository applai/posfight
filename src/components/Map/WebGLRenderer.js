 ...
 */
export default function WebGLRenderer(scenes, options){
    options = options || {};
    ...
    this.on("drawPointsChange",function(e){
        const g = that.drawShapeGraphics;
        const path = that.drawPoints;

        g.clear();

        const path2 = [];
        for(const j=0; j<path.length; j++){
            const v = path[j];
            path2.push([v[0], v[1]]);
        }
    ...
    this.on("drawCircleChange",function(e){
        const g = that.drawShapeGraphics;
        g.clear();
        const center = that.drawCircleCenter;
        const R = p2.vec2.dist(center, that.drawCirclePoint);
    ...
    this.on("drawRectangleChange",function(e){
        const g = that.drawShapeGraphics;
        g.clear();
        const start = that.drawRectStart;
        const end = that.drawRectEnd;
 ...
 */
const init_stagePosition = p2.vec2.create(),
...
WebGLRenderer.prototype.init = function(){
    ...
    const renderer =  this.renderer =     PIXI.autoDetectRenderer(s.width, s.height, null, null, true);
    const stage =     this.stage =        new PIXI.DisplayObjectContainer();
    const container = this.container =    new PIXI.Stage(0xFFFFFF,true);
    ...
    const lastX, lastY, lastMoveX, lastMoveY, startX, startY, down=false;

    const physicsPosA = p2.vec2.create();
    const physicsPosB = p2.vec2.create();
    const stagePos = p2.vec2.create();
    const initPinchLength = 0;
    const initScaleX = 1;
    const initScaleY = 1;
    const lastNumTouches = 0;
    container.mousedown = container.touchstart = function(e){
        ...
        if(e.originalEvent.touches && e.originalEvent.touches.length === 2){
            ...
            const touchA = that.container.interactionManager.touchs[0];
            const touchB = that.container.interactionManager.touchs[1];

            const pos = touchA.getLocalPosition(stage);
            p2.vec2.set(stagePos, pos.x, pos.y);
            that.stagePositionToPhysics(physicsPosA, stagePos);

            const pos = touchB.getLocalPosition(stage);
            p2.vec2.set(stagePos, pos.x, pos.y);
            that.stagePositionToPhysics(physicsPosB, stagePos);

            initPinchLength = p2.vec2.dist(physicsPosA, physicsPosB);

            const initScaleX = stage.scale.x;
            const initScaleY = stage.scale.y;
    ...
    container.mousemove = container.touchmove = function(e){
        ...
        if(e.originalEvent.touches && e.originalEvent.touches.length === 2){
            ...
            const pinchLength = p2.vec2.dist(physicsPosA, physicsPosB);
...
WebGLRenderer.prototype.zoom = function(x, y, zoomOut, actualScaleX, actualScaleY, multiplier){
    const scrollFactor = this.scrollFactor,
        stage = this.stage;
...
WebGLRenderer.prototype.drawConvex = function(g,verts,triangles,color,fillColor,lineWidth,debug,offset,isSleeping){
    ...
    if(!debug){
        ...
        for(const i=0; i!==verts.length+1; i++){
...
WebGLRenderer.prototype.render = function(){
    ...
    for(const i=0; i!==this.bodies.length; i++){
        const isSleeping = (this.bodies[i].sleepState===p2.Body.SLEEPING);
    ...
    for(const i=0; i!==this.springs.length; i++){
    ...
    if(this.drawContacts){
        ...
        for(const i=0; i!==this.world.narrowphase.contactEquations.length; i++){
    ...
    if(this.drawAABBs){
        ...
        for(const i=0; i!==this.world.bodies.length; i++){
        ...
        if(obj.concavePath && !this.debugPolygons){
            ...
            for(const i=0; i<obj.shapes.length; i++){
                ...
                if(child instanceof p2.Circle){
                    ...
                    const verts = [],
                        vrot = p2.vec2.create();
                    for(const j=0; j!==child.vertices.length; j++){
                    ...
                    const path = [[0,-100]];
                    for(const j=0; j!==child.heights.length; j++){
